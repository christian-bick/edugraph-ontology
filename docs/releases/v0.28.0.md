...
The convention is defined in [ONT-D4](https://github.com/christian-bick/edugraph-ontology/blob/v0.28.0/docs/descriptors.md#ont-d4--define-the-educational-meaning-and-its-boundaries).
...
[constructing statements](https://github.com/christian-bick/edugraph-ontology/blob/v0.28.0/docs/annotations-and-models.md#constructing-statements-from-descriptor-text).
...
Review definition-dependent uses under [ONT-W2](https://github.com/christian-bick/edugraph-ontology/blob/v0.28.0/docs/change-review.md#ont-w2--review-definitions-relations-and-effects-together),
especially these clarifications:

- `MetricDistanceScale` covers metric units of length rather than only meters, with
  unit examples supplied in its comment.
...
conflict semantics in [ONT-R2](https://github.com/christian-bick/edugraph-ontology/blob/v0.28.0/docs/relations.md#ont-r2--interpret-constraints-at-the-descriptor-level).
The pre-existing unit-fraction endpoint conflict and adoption of comments in client consumers
remain in [consolidation tracking](https://github.com/christian-bick/edugraph-ontology/blob/v0.28.0/docs/plan/ontology-consolidation.md). The shared snapshot APIs
...
- A focused N3-parsed source comparison confirms that all 2,890 non-annotation assertions are
  unchanged from v0.27.0. Only `rdfs:isDefinedBy` and `rdfs:comment` differ in the ontology sources.
...
The tagged workflow repeats the build and checks Python 3.11–3.14 on Linux and Python 3.14
on Windows before publishing. Assets include the four Turtle sources, combined RDF/XML,
`edugraph-ts.tgz` with package version 0.28.0, `edugraph_py-0.28.0-py3-none-any.whl`, and
`edugraph_py-0.28.0.tar.gz`. Client APIs and supported Python versions are unchanged.
