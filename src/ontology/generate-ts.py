...
def generate_relations_file(ontology, output_dir, individual_to_class):
    ...
    content += ' * implies/impliedBy closures) contains a contradiction edge. Bounds (e.g.\n'
    content += ' * numeric range limits) come in contradicting pairs, so compatibility for\n'
    content += ' * them traverses towards tighter labels (impliedBy), while contradiction-free\n'
    content += ' * families propagate capabilities along implies.\n'
    content += ' */\n'
    content += 'function isBoundTyped(descriptor: CompetencyDescriptor): boolean {\n'
    content += '  if (contradicts(descriptor).length > 0) return true;\n'
    content += '  return [...impliesTransitive(descriptor), ...impliedByTransitive(descriptor)]\n'
    content += '    .some(related => contradicts(related).length > 0);\n'
    content += '}\n\n'
    content += '/**\n'
    ...
    content += ' * Note this composes implies with contradicts — contradictsTransitive alone\n'
    ...
    content += ' */\n'
    content += 'export function incompatible(a: CompetencyDescriptor, b: CompetencyDescriptor): boolean {\n'
    content += '  const aClosure = [a, ...impliesTransitive(a)];\n'
    content += '  const bClosure = new Set<CompetencyDescriptor>([b, ...impliesTransitive(b)]);\n'
    content += '  return aClosure.some(x => contradicts(x).some(y => bClosure.has(y)));\n'
    content += '}\n'
    ...
    content += '    const transitiveCollection = isBoundTyped(constraint)\n'
    content += '      ? impliedByTransitive(constraint)\n'
    content += '      : impliesTransitive(constraint);\n'
    content += '    for (const imp of transitiveCollection) {\n'
    ...
    content += '      const transitiveCollection = isBoundTyped(c)\n'
    content += '        ? impliedByTransitive(c)\n'
    content += '        : impliesTransitive(c);\n'
    content += '      for (const imp of transitiveCollection) {\n'
    ...
    content += ' *\n'
    content += ' * Dual of deductCompatible: capabilities are declared with deductCompatible\n'
    content += ' * (labels guaranteed to stay within a declared window, conjunctive across\n'
    content += ' * constraints), while boundaries are declared with deductAdmitting (labels\n'
    content += ' * that permit content beyond a line, disjunctive across boundaries).\n'
    content += ' *\n'
    content += ' * Typical use is declaring rejection boundaries, e.g. a view that cannot\n'
    content += ' * render numbers beyond 10 rejects every scope admitting such content:\n'
    content += ' * `rejectedLabels: [...deductAdmitting([Scope.NumbersLarger10])]`.\n'
    content += ' *\n'
    content += ' * For each boundary B the result is the union of:\n'
    content += ' * - B and all labels implying B: their content must cross the line;\n'
    content += ' * - the weakenings of B\'s contradiction partners: bounds loose enough\n'
    content += ' *   that their content may cross the line.\n'
    content += ' * Labels B implies (pure opposite-direction bounds) and B\'s contradiction\n'
    content += ' * partners themselves (which guarantee staying within the line) are spared.\n'
    content += ' */\n'
    content += 'export function deductAdmitting(boundaries: CompetencyDescriptor[]): CompetencyDescriptor[] {\n'
    content += '  const admitting = new Set<CompetencyDescriptor>();\n'
    content += '  for (const boundary of boundaries) {\n'
    content += '    admitting.add(boundary);\n'
    content += '    for (const tighter of impliedByTransitive(boundary)) {\n'
    content += '      admitting.add(tighter);\n'
    content += '    }\n'
    content += '    for (const partner of contradicts(boundary)) {\n'
    content += '      for (const weaker of impliesTransitive(partner)) {\n'
    content += '        admitting.add(weaker);\n'
    content += '      }\n'
    content += '    }\n'
    content += '  }\n'
    content += '  return Array.from(admitting);\n'
    content += '}\n\n'
